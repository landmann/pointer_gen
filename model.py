  ...
  def _add_seq2seq(self):
    ...
    with tf.variable_scope('seq2seq'):
      ...
      with tf.variable_scope('embedding'):
        if hps.emb_path:
            # Load embedding weights if path is provided.
            import pickle
            weights = pickle.load(open(hps.emb_path, 'rb'))[:vsize]
            weights = np.asarray(weights, dtype=np.float32)
            weights_initializer = tf.constant_initializer(weights)
            embedding = tf.get_variable('embedding', [vsize, hps.emb_dim], dtype=tf.float32, initializer = weights_initializer, trainable=False)
        else:
          ...
          if FLAGS.pointer_gen:
            ...
            for dec_step, dist in enumerate(final_dists):
              ...
              if hps.loss_type=='nll':
                ...
                raise ValueError("FLAGS.loss_type must be either 'nll', 'emb_loss")

            # Apply dec_padding_mask and get loss
            # The mask serves to average by the number of words in the decoder (not using the padding)
